...
type DeploymentEventWatcherFactory interface {
	Build(mgr manager.AsyncManager, clusterName string) k8s_apps_controller.DeploymentEventWatcher
}
...
type PodEventWatcherFactory interface {
	Build(mgr manager.AsyncManager, clusterName string) k8s_core_controller.PodEventWatcher
}

type MeshEventWatcherFactory interface {
	Build(mgr manager.AsyncManager, clusterName string) zephyr_discovery_controller.MeshEventWatcher
}

type MeshWorkloadEventWatcherFactory interface {
	Build(mgr manager.AsyncManager, clusterName string) zephyr_discovery_controller.MeshWorkloadEventWatcher
}

type ServiceEventWatcherFactory interface {
	Build(mgr manager.AsyncManager, clusterName string) k8s_core_controller.ServiceEventWatcher
}
