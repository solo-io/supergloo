...
// a `MeshDiscoverer` receives deployment events from a controller that it gets attached to
// this is the abstraction that should be directly managing the Mesh CR instances, based on what
// its `meshFinders` determine
func NewMeshDiscoverer(
	ctx context.Context,
	clusterName string,
	meshFinders []MeshFinder,
	localMeshClient LocalMeshClient,
) controller.DeploymentEventHandler {
	return &meshDiscoverer{
...
func (d *meshDiscoverer) Create(deployment *k8s_api_v1.Deployment) error {
	logger := d.buildEventLogger(createEvent)
...
func (d *meshDiscoverer) Update(old, new *k8s_api_v1.Deployment) error {
	logger := d.buildEventLogger(updateEvent)
...
func (d *meshDiscoverer) Delete(deployment *k8s_api_v1.Deployment) error {
	// TODO: Not deleting any entities for now
	return nil
}

func (d *meshDiscoverer) Generic(deployment *k8s_api_v1.Deployment) error {
	// not implemented- we haven't implemented generic events for this controller
	d.logger.Warn("Unexpected generic event received by mesh discovery")
	return nil
}

func (d *meshDiscoverer) buildEventLogger(eventType eventType) *zap.SugaredLogger {
	enhancedLogger := d.logger

	switch eventType {
	case createEvent:
		enhancedLogger = enhancedLogger.With("event_type", "create")
	case updateEvent:
		enhancedLogger = enhancedLogger.With("event_type", "update")
	case deleteEvent:
		enhancedLogger = enhancedLogger.With("event_type", "delete")
	}

	return enhancedLogger.With("cluster_name", d.clusterName)
}

// if both `discoveredMesh` and `err` are non-nil, then `err` should be considered a non-fatal error
func (d *meshDiscoverer) discoverMesh(deployment *k8s_api_v1.Deployment) (discoveredMesh *v1alpha1.Mesh, err error) {
	var multiErr *multierror.Error
	for _, meshFinder := range d.meshFinders {
...
func (d *meshDiscoverer) createIfNotExists(discoveredMesh *v1alpha1.Mesh) error {
	_, err := d.localMeshClient.Get(d.ctx, client.ObjectKey{
		Namespace: discoveredMesh.GetNamespace(),
		Name:      discoveredMesh.GetName(),
	})
...
func (d *meshDiscoverer) delete(mesh *v1alpha1.Mesh) error {
