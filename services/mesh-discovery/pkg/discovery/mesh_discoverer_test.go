...
var _ = Describe("Mesh Discoverer", func() {
	var (
	...
	Context("Create Event", func() {
		It("can discover a mesh", func() {
			meshFinder := mock_discovery.NewMockMeshFinder(ctrl)
			...
			eventHandler := discovery.NewMeshDiscoverer(
				ctx,
				clusterName,
				[]discovery.MeshFinder{meshFinder},
				localMeshClient,
			)
		...
		It("can go on to discover a mesh if one of the other finders errors out", func() {
			meshFinder := mock_discovery.NewMockMeshFinder(ctrl)
			brokenMeshFinder := mock_discovery.NewMockMeshFinder(ctrl)
			localMeshClient := mock_discovery.NewMockLocalMeshClient(ctrl)
			deployment := mock_discovery.BuildDeployment(v1.ObjectMeta{Name: "test-deployment", Namespace: remoteNamespace})
			meshObjectMeta := v1.ObjectMeta{Name: "test-mesh", Namespace: remoteNamespace}
			mesh := mock_discovery.BuildMesh(meshObjectMeta)
			...
			eventHandler := discovery.NewMeshDiscoverer(
				ctx,
				clusterName,
				[]discovery.MeshFinder{brokenMeshFinder, meshFinder},
				localMeshClient,
			)
