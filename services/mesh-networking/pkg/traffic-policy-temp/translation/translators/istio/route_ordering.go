...
func (b SpecificitySortableRoutes) Less(i, j int) bool {
	...
	return b.isHttpRouteMatcherMoreSpecific(b[i], b[j])
}
...
func (s SpecificitySortableRoutes) isHttpRouteMatcherMoreSpecific(httpRouteA, httpRouteB *istio_networking.HTTPRoute) bool {
	...
	if s.isStringMatchMoreSpecific(a.GetUri(), b.GetUri()) {
		return true
	} else if s.isStringMatchMoreSpecific(b.GetUri(), a.GetUri()) {
		return false
	}
	if s.isMethodMoreSpecific(a.GetMethod(), b.GetMethod()) {
		return true
	} else if s.isMethodMoreSpecific(b.GetMethod(), a.GetMethod()) {
		return false
	}
...
func (SpecificitySortableRoutes) isStringMatchMoreSpecific(a, b *istio_networking.StringMatch) bool {
...
func (s SpecificitySortableRoutes) isMethodMoreSpecific(a, b *istio_networking.StringMatch) bool {
	if a.GetExact() == "" {
		return false
	} else if b.GetExact() == "" {
		return true
