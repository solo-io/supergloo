...
import (
	...
	mock_csr_agent_controller "github.com/solo-io/mesh-projects/services/csr-agent/pkg/controller/mocks"
...
var _ = Describe("csr processor", func() {
	var (
		...
		csrProcessor security_processors.MeshGroupCertificateSigningRequestProcessor
	...
	BeforeEach(func() {
		...
		csrProcessor = csr_agent_controller.NewCsrAgentIstioProcessor(csrClient, secretClient, certClient, signer)
	})
	...
	It("will return an error if secret key cannot be ensured", func() {
		...
		status := csrProcessor.ProcessCreate(ctx, csr)
		Expect(status).To(Equal(security_types.MeshGroupCertificateSigningRequestStatus{
			ComputedStatus: &core_types.ComputedStatus{
				...
				Message: csr_agent_controller.FailedToRetrievePrivateKeyError(testErr).Error(),
			},
		}))
	})
	...
	Context("no csr", func() {
		...
		It("will return an error if csr cannot be generated", func() {
			...
			signer.EXPECT().
				GenCSRWithKey(pki_util.CertOptions{
					...
					Org:           csr.Spec.GetOrg(),
					Host:          strings.Join(csr.Spec.GetHosts(), ","),
				}).
				Return(nil, testErr)
			...
			Expect(status).To(Equal(security_types.MeshGroupCertificateSigningRequestStatus{
				ComputedStatus: &core_types.ComputedStatus{
					Status:  core_types.ComputedStatus_INVALID,
					Message: csr_agent_controller.FailedToGenerateCSRError(testErr).Error(),
				},
			}))
		})
		...
		It("will return an error mgcsr cannot be updated with csr bytes", func() {
			...
			Expect(status).To(Equal(security_types.MeshGroupCertificateSigningRequestStatus{
				ComputedStatus: &core_types.ComputedStatus{
					Status:  core_types.ComputedStatus_INVALID,
					Message: csr_agent_controller.FailesToAddCsrToResource(testErr).Error(),
				},
			}))
		})
	...
	Context("with cert data", func() {
		...
		It("will fail if istio CA cannot be updated", func() {
			...
			secretClient.EXPECT().
				Get(ctx, csr_agent_controller.IstioCaSecretName, "istio-system").
				Return(nil, testErr)

			matchErr := csr_agent_controller.FailedToUpdateCaError(testErr)
			status := csrProcessor.ProcessCreate(ctx, csr)
			Expect(status).To(Equal(security_types.MeshGroupCertificateSigningRequestStatus{
				ComputedStatus: &core_types.ComputedStatus{
					Status:  core_types.ComputedStatus_INVALID,
		...
		It("will create a new resource if the secret doesn't exist", func() {
			...
			secretClient.EXPECT().
				Create(ctx, certData.BuildSecret(csr_agent_controller.IstioCaSecretName, "istio-system")).
				Return(testErr)
		...
		It("won't try to update istio CA if it already exists, and the content is equal", func() {
			...
			secretClient.EXPECT().
				Get(ctx, csr_agent_controller.IstioCaSecretName, "istio-system").
				Return(certData.BuildSecret(csr_agent_controller.IstioCaSecretName, "istio-system"), nil)
		...
		It("will update cacerts with new data if it is different", func() {
			...
			secretClient.EXPECT().
				Update(ctx, matchCert.BuildSecret(csr_agent_controller.IstioCaSecretName, "istio-system")).
				Return(nil)
