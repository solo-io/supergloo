...
import (
	...
	. "github.com/solo-io/service-mesh-hub/services/common/mesh-platform/k8s"
	. "github.com/solo-io/service-mesh-hub/services/common/mesh-platform/k8s/mocks"
...
var _ = Describe("mc_manager", func() {
	var (
		...
		configHandler     mesh_platform.MeshPlatformCredentialsHandler
		...
		Context("no handlers", func() {
			Context("add", func() {
				It("will ignore non k8s cluster secret", func() {
					...
					err := configHandler.MeshPlatformAdded(ctx, secret)
					Expect(err).To(BeNil())
				})
			...
			Context("remove", func() {
				It("will ignore non k8s secrets", func() {
					...
					err := configHandler.MeshPlatformRemoved(ctx, secret)
					Expect(err).To(BeNil())
				})
