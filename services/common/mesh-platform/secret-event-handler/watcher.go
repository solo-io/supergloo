...
import (
	...
	internal_watcher "github.com/solo-io/service-mesh-hub/services/common/mesh-platform/secret-event-handler/internal"
...
func StartLocalManager(
	...
	return func(ctx context.Context, mgr manager.Manager) error {
		...
		mcHandler := &meshPlatformHandler{
			ctx:                           ctx,
			meshPlatformMembershipHandler: internal_watcher.NewMeshPlatformMembershipHandler(meshPlatformCredentialsHandlers),
		}
...
func (c *meshPlatformHandler) CreateSecret(s *core_v1.Secret) error {
	resync, err := c.meshPlatformMembershipHandler.MeshPlatformSecretAdded(c.ctx, s)
	if err != nil {
...
func (c *meshPlatformHandler) UpdateSecret(old, new *core_v1.Secret) error {
	...
	if internal_watcher.HasRequiredMetadata(old.GetObjectMeta(), old) && !internal_watcher.HasRequiredMetadata(new.GetObjectMeta(), new) {
		resync, err := c.meshPlatformMembershipHandler.MeshPlatformSecretRemoved(c.ctx, new)
		if err != nil {
	...
	if !internal_watcher.HasRequiredMetadata(old.GetObjectMeta(), old) && internal_watcher.HasRequiredMetadata(new.GetObjectMeta(), new) {
		resync, err := c.meshPlatformMembershipHandler.MeshPlatformSecretAdded(c.ctx, new)
		if err != nil {
...
func (c *meshPlatformHandler) DeleteSecret(s *core_v1.Secret) error {
	resync, err := c.meshPlatformMembershipHandler.MeshPlatformSecretRemoved(c.ctx, s)
	if err != nil {
