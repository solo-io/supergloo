...
var _ = Describe("mc_manager", func() {
	var (
		...
		configHandler   KubeConfigHandler
		...
		Context("no handlers", func() {
			...
			Context("delete", func() {
				It("will return an error if get manager fails", func() {
					...
					Expect(err).To(HaveOccurred())
					Expect(err).To(HaveInErrorChain(NoManagerForClusterError(clusterName)))
				})
			})
		})
		...
		Context("with handlers", func() {
			...
			Context("delete", func() {
				...
				BeforeEach(func() {
					...
					Expect(err).NotTo(HaveOccurred())
					managerController := NewAsyncManagerController(ctx, handlerMap,
