...
var _ = Describe("IstioCacertsSecretClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs IstioCacertsSecrets "+test.Description(), func() {
				IstioCacertsSecretClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func IstioCacertsSecretClientTest(namespace string, client IstioCacertsSecretClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
	...
	Eventually(func() IstioCacertsSecretList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
