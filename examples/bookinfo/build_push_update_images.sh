...
do
  IMAGES+=$(docker images -f reference=soloio/examples-bookinfo*:"$v" --format "{{.Repository}}:$v")
  IMAGES+=" "
done
...
find . -name "*bookinfo*.yaml" -exec sed -i.bak "s/\\(soloio\\/examples-bookinfo-.*\\):[[:digit:]]*\\.[[:digit:]]*\\.[[:digit:]]*/\\1:$VERSION/g" {} +
