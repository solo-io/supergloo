...
commentsHostname = "comments" if (os.environ.get("COMMENTS_HOSTNAME") == None) else os.environ.get("COMMENTS_HOSTNAME")
...
comments = {
    "name" : "http://{0}{1}:9080".format(commentsHostname, servicesDomain),
    "endpoint" : "comments",
    "children" : []
}
...
productpage = {
    "name" : "http://{0}{1}:9080".format(detailsHostname, servicesDomain),
    "endpoint" : "details",
    "children" : [details, comments, reviews]
}
...
def front():
    ...
    commentsStatus, comments = getProductComments(product_id, headers)
...
@app.route('/api/v1/products/<product_id>/comments')
@trace()
def commentsRoute(product_id):
    headers = getForwardHeaders(request)
    status, comments = getProductComments(product_id, headers)
    return json.dumps(comments), status, {'Content-Type': 'application/json'}
...
def getProductComments(product_id, headers):
    try:
        url = comments['name'] + "/" + comments['endpoint'] + "/" + str(product_id)
        res = requests.get(url, headers=headers, timeout=3.0)
    except:
        res = None
    if res and res.status_code == 200:
        ...
        status = res.status_code if res is not None and res.status_code else 500
        return status, {'error': 'Sorry, product comments are currently unavailable for this book.'}
