...
| egressGateways | []discovery.mesh.gloo.solo.io.MeshSpec.Istio.EgressGatewayInfo | repeated | Configuration metadata for Istio EgressGateway (the Istio Egress). |
...
| workloadLabels | []discovery.mesh.gloo.solo.io.MeshSpec.Istio.EgressGatewayInfo.WorkloadLabelsEntry | repeated | Labels matching the workload which backs the gateway, defaults to `{"istio": "egressgateway"}`. |
