...
Let's install Istio into both the **both** clusters. The configuration below assumes that you are using the Kind clusters created in the Setup guide. The contexts for those clusters should be `kind-cluster1` and `kind-cluster2`. Both clusters are using a NodePort to expose the ingress gateway for Istio. If you are deploying Istio on different cluster setup, update your context and gateway settings accordingly.
...
With Gloo Mesh and Istio installed into `cluster1`, and Istio installed into `cluster2`, we have an architecture that looks like this:
