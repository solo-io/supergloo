...
We can see the certificate chain used to establish mTLS between Istio services in both clusters and can compare them to be different. One way to see the certificates, is to use the `openssl s_client` tool with the `-showcerts` param when calling between two services. Let's try it on `cluster-1`:
...
Lastly, we are creating the VirtualMesh with two different service meshes: `istiod-istio-system-cluster-1` and `istiod-istio-system-cluster-2`. We can have any meshes defined here that should be part of this virtual grouping and federation.
