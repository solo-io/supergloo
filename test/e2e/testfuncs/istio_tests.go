...
func RunIstioE2eTests(params IstioE2eTestParams) {
	...
	log.Printf("istio config: %#v", opts)

	kube := params.Kube

	By("it installs istio", func() {
		testInstallIstio(kube, params.MeshName, params.BasicNamespace, params.NamespaceWithInject, params.IstioNamespace, params.SuperglooNamespace)
	})

	By("discovers and configures smi adapter", func() {
		err := sgutils.KubectlApplyFile(params.SmiIstioAdapterFile)
		...
		testTrafficShifting(params.RootCtx, params.MeshName, params.NamespaceWithInject, params.SuperglooNamespace)
		err = sgutils.KubectlDeleteFile(params.SmiIstioAdapterFile)
		Expect(err).NotTo(HaveOccurred())
	})
	...
	By("it enforces policy", func() {
		testPolicy(params.RootCtx, params.MeshName, params.NamespaceWithInject, params.SuperglooNamespace)
	})

	By("it configures prometheus", func() {
		testConfigurePrometheus(params.RootCtx, params.SharedLock, params.MeshName, params.BasicNamespace, params.PromNamespace)
	})

	By("it enables mtls", func() {
		testMtls(params.RootCtx, params.BasicNamespace, params.NamespaceWithInject)
	})

	By("it enables mtls ingress with gloo", func() {
		testGlooMtls(params.RootCtx, params.MeshName, params.GlooNamespace, params.BasicNamespace, params.NamespaceWithInject, params.SuperglooNamespace)
	})

	By("it sets custom root ca", func() {
		testCertRotation(params.RootCtx, params.MeshName, params.NamespaceWithInject)
	})
	...
	By("it injects faults with routing rules", func() {
		testFaultInjection(params.RootCtx, params.MeshName, params.NamespaceWithInject, params.SuperglooNamespace)
	})

	By("it uninstalls gloo", func() {
		testUninstallGloo(glooName, params.SuperglooNamespace)
	})

	By("it uninstalls istio", func() {
		testUninstallIstio(kube, params.MeshName, params.IstioNamespace, params.SuperglooNamespace)
	})
}
...
/*
   tests
*/
func testInstallIstio(kube kubernetes.Interface, meshName, basicNamespace, namespaceWithInject, istioNamespace, superglooNamespace string) {
...
func testCertRotation(rootCtx context.Context, meshName, namespaceWithInject string) {
...
func testMtls(rootCtx context.Context, basicNamespace, namespaceWithInject string) {
...
func testGlooMtls(rootCtx context.Context, meshName, glooNamespace, basicNamespace, namespaceWithInject, superglooNamespace string) {
	service := "details"
	port := 9080
	...
	err := utils.Supergloo(fmt.Sprintf("set upstream mtls --name %s --target-mesh %s.%s",
		upstreamName, superglooNamespace, meshName))
	Expect(err).NotTo(HaveOccurred())

	sgutils.TestGlooIngress(rootCtx, namespaceWithInject, superglooNamespace, glooNamespace, basicNamespace)
}

func testPolicy(rootCtx context.Context, meshName, namespaceWithInject, superglooNamespace string) {
...
func testTrafficShifting(rootCtx context.Context, meshName, namespaceWithInject, superglooNamespace string) {
...
func testFaultInjection(rootCtx context.Context, meshName, namespaceWithInject, superglooNamespace string) {
...
func testUninstallIstio(kube kubernetes.Interface, meshName, istioNamespace, superglooNamespace string) {
...
func testConfigurePrometheus(rootCtx context.Context, lock *sync.Mutex, meshName, basicNamespace, promNamespace string) {
	lock.Lock()
	defer lock.Unlock()
...
util funcs
*/
func testUninstallGloo(meshIngressName, superglooNamespace string) {
	// test uninstall works
