...
var _ = Describe("ResourceSelector", func() {
	...
	Describe("GetMeshWorkloadsForMeshService", func() {
		It("should return MeshWorkloads selected by MeshService", func() {
			...
			meshWorkloadList := &zephyr_discovery.MeshWorkloadList{
				Items: []zephyr_discovery.MeshWorkload{
					{
						ObjectMeta: k8s_meta_types.ObjectMeta{
							Name: "n1", Namespace: "ns1",
						},
						Spec: zephyr_discovery_types.MeshWorkloadSpec{
							KubeController: &zephyr_discovery_types.MeshWorkloadSpec_KubeController{
								Labels: map[string]string{
			...
			mockMeshWorkloadClient.EXPECT().ListMeshWorkload(ctx).Return(meshWorkloadList, nil)
			expectedMeshWorkloads := []*zephyr_discovery.MeshWorkload{
				&meshWorkloadList.Items[0], &meshWorkloadList.Items[1],
			}
			workloads, err := resourceSelector.GetMeshWorkloadsForMeshService(ctx, meshService)
			Expect(err).To(BeNil())
			Expect(workloads).To(ConsistOf(expectedMeshWorkloads))
		})
	})
})
