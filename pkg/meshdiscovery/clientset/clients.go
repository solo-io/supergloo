...
func newInputClients(pod skkube.PodClient, namespace skkube.KubeNamespaceClient, configMap skkube.ConfigMapClient,
	meshIngress v1.MeshIngressClient, upstream gloov1.UpstreamClient, secret gloov1.SecretClient) *inputClients {
	return &inputClients{Pod: pod, Namespace: namespace, ConfigMap: configMap, MeshIngress: meshIngress,
...
func ClientsetFromContext(ctx context.Context) (*Clientset, error) {
	...
	*/
	...
	return newClientset(
		restConfig,
		kubeClient,
		newInputClients(pods, namespace, configMap, meshIngress, upstream, secret),
