...
var _ = Describe("AppmeshDiscoveryEventLoop", func() {
	var (
		namespace string
	...
	BeforeEach(func() {
		...
		emitter = NewAppmeshDiscoveryEmitter(meshClient, podClient, upstreamClient)
	})
	It("runs sync function on a new snapshot", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		sync := &mockAppmeshDiscoverySyncer{}
		el := NewAppmeshDiscoveryEventLoop(emitter, sync)
...
type mockAppmeshDiscoverySyncer struct {
	synced bool
	mutex  sync.Mutex
}

func (s *mockAppmeshDiscoverySyncer) Synced() bool {
	s.mutex.Lock()
	defer s.mutex.Unlock()
	return s.synced
}

func (s *mockAppmeshDiscoverySyncer) Sync(ctx context.Context, snap *AppmeshDiscoverySnapshot) error {
	s.mutex.Lock()
	s.synced = true
	s.mutex.Unlock()
	return nil
