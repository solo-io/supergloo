...
type RbacSyncer interface {
	Sync(context.Context, *RbacSnapshot) error
}

type RbacSyncers []RbacSyncer

func (s RbacSyncers) Sync(ctx context.Context, snapshot *RbacSnapshot) error {
...
type rbacEventLoop struct {
	emitter RbacSnapshotEmitter
	syncer  RbacSyncer
	ready   chan struct{}
}

func NewRbacEventLoop(emitter RbacSnapshotEmitter, syncer RbacSyncer) eventloop.EventLoop {
	return &rbacEventLoop{
		emitter: emitter,
		syncer:  syncer,
...
func (el *rbacEventLoop) Ready() <-chan struct{} {
	return el.ready
}

func (el *rbacEventLoop) Run(namespaces []string, opts clients.WatchOpts) (<-chan error, error) {
	...
	go func() {
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				ctx, span := trace.StartSpan(opts.Ctx, "rbac.zephyr.solo.io.EventLoopSync")
