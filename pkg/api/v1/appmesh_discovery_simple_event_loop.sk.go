...
type AppmeshDiscoverySyncDecider interface {
	AppmeshDiscoverySyncer
	ShouldSync(old, new *AppmeshDiscoverySnapshot) bool
}

type appmeshDiscoverySimpleEventLoop struct {
	emitter AppmeshDiscoverySimpleEmitter
	syncers []AppmeshDiscoverySyncer
}

func NewAppmeshDiscoverySimpleEventLoop(emitter AppmeshDiscoverySimpleEmitter, syncers ...AppmeshDiscoverySyncer) eventloop.SimpleEventLoop {
	return &appmeshDiscoverySimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *appmeshDiscoverySimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[AppmeshDiscoverySyncer]context.CancelFunc)
		...
		var previousSnapshot *AppmeshDiscoverySnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(AppmeshDiscoverySyncDecider); isDecider {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("appmeshDiscovery.supergloo.solo.io.SimpleEventLoopSync-%T", syncer))
