...
type LinkerdDiscoverySyncDecider interface {
	LinkerdDiscoverySyncer
	ShouldSync(old, new *LinkerdDiscoverySnapshot) bool
}

type linkerdDiscoverySimpleEventLoop struct {
	emitter LinkerdDiscoverySimpleEmitter
	syncers []LinkerdDiscoverySyncer
}

func NewLinkerdDiscoverySimpleEventLoop(emitter LinkerdDiscoverySimpleEmitter, syncers ...LinkerdDiscoverySyncer) eventloop.SimpleEventLoop {
	return &linkerdDiscoverySimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *linkerdDiscoverySimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[LinkerdDiscoverySyncer]context.CancelFunc)
		...
		var previousSnapshot *LinkerdDiscoverySnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(LinkerdDiscoverySyncDecider); isDecider {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("linkerdDiscovery.supergloo.solo.io.SimpleEventLoopSync-%T", syncer))
