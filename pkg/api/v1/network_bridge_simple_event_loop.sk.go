...
// Deprecated: use NetworkBridgeSyncDeciderWithContext
type NetworkBridgeSyncDecider interface {
	NetworkBridgeSyncer
	ShouldSync(old, new *NetworkBridgeSnapshot) bool
}

type NetworkBridgeSyncDeciderWithContext interface {
	NetworkBridgeSyncer
	ShouldSync(ctx context.Context, old, new *NetworkBridgeSnapshot) bool
}

type networkBridgeSimpleEventLoop struct {
	emitter NetworkBridgeSimpleEmitter
	syncers []NetworkBridgeSyncer
}

func NewNetworkBridgeSimpleEventLoop(emitter NetworkBridgeSimpleEmitter, syncers ...NetworkBridgeSyncer) eventloop.SimpleEventLoop {
	return &networkBridgeSimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *networkBridgeSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[NetworkBridgeSyncer]context.CancelFunc)
		...
		var previousSnapshot *NetworkBridgeSnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(NetworkBridgeSyncDecider); isDecider {
						if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
							continue // skip syncing this syncer
						}
					} else if syncDeciderWithContext, isDecider := syncer.(NetworkBridgeSyncDeciderWithContext); isDecider {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("network_bridge.zephyr.solo.io.SimpleEventLoopSync-%T", syncer))
