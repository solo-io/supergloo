...
type configSimpleEventLoop struct {
	emitter ConfigSimpleEmitter
	syncers []ConfigSyncer
}

func NewConfigSimpleEventLoop(emitter ConfigSimpleEmitter, syncers ...ConfigSyncer) eventloop.SimpleEventLoop {
	return &configSimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *configSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[ConfigSyncer]context.CancelFunc)
		...
		var previousSnapshot *ConfigSnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(ConfigSyncDecider); isDecider {
						if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
							continue // skip syncing this syncer
						}
					} else if syncDeciderWithContext, isDecider := syncer.(ConfigSyncDeciderWithContext); isDecider {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("config.supergloo.solo.io.SimpleEventLoopSync-%T", syncer))
