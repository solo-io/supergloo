...
// Deprecated: use RbacSyncDeciderWithContext
type RbacSyncDecider interface {
	RbacSyncer
	ShouldSync(old, new *RbacSnapshot) bool
}

type RbacSyncDeciderWithContext interface {
	RbacSyncer
	ShouldSync(ctx context.Context, old, new *RbacSnapshot) bool
}

type rbacSimpleEventLoop struct {
	emitter RbacSimpleEmitter
	syncers []RbacSyncer
}

func NewRbacSimpleEventLoop(emitter RbacSimpleEmitter, syncers ...RbacSyncer) eventloop.SimpleEventLoop {
	return &rbacSimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *rbacSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[RbacSyncer]context.CancelFunc)
		...
		var previousSnapshot *RbacSnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(RbacSyncDecider); isDecider {
						if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
							continue // skip syncing this syncer
						}
					} else if syncDeciderWithContext, isDecider := syncer.(RbacSyncDeciderWithContext); isDecider {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("rbac.zephyr.solo.io.SimpleEventLoopSync-%T", syncer))
