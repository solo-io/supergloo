...
type registrationSimpleEventLoop struct {
	emitter RegistrationSimpleEmitter
	syncers []RegistrationSyncer
}
...
func NewRegistrationSimpleEventLoop(emitter RegistrationSimpleEmitter, syncers ...RegistrationSyncer) eventloop.SimpleEventLoop {
	return &registrationSimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *registrationSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[RegistrationSyncer]context.CancelFunc)
		...
		var previousSnapshot *RegistrationSnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					if syncDecider, isDecider := syncer.(RegistrationSyncDecider); isDecider {
						if shouldSync := syncDecider.ShouldSync(previousSnapshot, snapshot); !shouldSync {
							continue // skip syncing this syncer
						}
					} else if syncDeciderWithContext, isDecider := syncer.(RegistrationSyncDeciderWithContext); isDecider {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("registration.supergloo.solo.io.SimpleEventLoopSync-%T", syncer))
