...
var _ = Describe("MeshIngressClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: MeshIngressCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewMeshIngressClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs MeshIngresss "+test.Description(), func() {
				MeshIngressClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func MeshIngressClientTest(namespace string, client MeshIngressClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewMeshIngress(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&MeshIngress{}))
	...
	Eventually(func() MeshIngressList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
