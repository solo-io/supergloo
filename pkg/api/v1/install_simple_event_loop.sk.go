...
type installSimpleEventLoop struct {
	emitter InstallSimpleEmitter
	syncers []InstallSyncer
}

func NewInstallSimpleEventLoop(emitter InstallSimpleEmitter, syncers ...InstallSyncer) eventloop.SimpleEventLoop {
	return &installSimpleEventLoop{
		emitter: emitter,
		syncers: syncers,
	}
}

func (el *installSimpleEventLoop) Run(ctx context.Context) (<-chan error, error) {
	...
	go func() {
		...
		syncerCancels := make(map[InstallSyncer]context.CancelFunc)
		...
		var previousSnapshot *InstallSnapshot
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				for _, syncer := range el.syncers {
					...
					ctx, span := trace.StartSpan(ctx, fmt.Sprintf("install.supergloo.solo.io.SimpleEventLoopSync-%T", syncer))
