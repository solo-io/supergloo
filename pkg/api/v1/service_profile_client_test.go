...
var _ = Describe("ServiceProfileClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewServiceProfileClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs ServiceProfiles "+test.Description(), func() {
				ServiceProfileClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func ServiceProfileClientTest(namespace string, client ServiceProfileClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewServiceProfile(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&ServiceProfile{}))
	...
	Eventually(func() ServiceProfileList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
