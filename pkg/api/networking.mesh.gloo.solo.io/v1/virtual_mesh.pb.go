// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.25.0
// 	protoc        v3.6.1
// source: github.com/solo-io/gloo-mesh/api/networking/v1/virtual_mesh.proto

package v1

import (
	proto "github.com/golang/protobuf/proto"
	empty "github.com/golang/protobuf/ptypes/empty"
	_ "github.com/golang/protobuf/ptypes/wrappers"
	v11 "github.com/solo-io/gloo-mesh/pkg/api/common.mesh.gloo.solo.io/v1"
	_ "github.com/solo-io/protoc-gen-ext/extproto"
	v1 "github.com/solo-io/skv2/pkg/api/core.skv2.solo.io/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// This is a compile-time assertion that a sufficiently up-to-date version
// of the legacy proto package is being used.
const _ = proto.ProtoPackageIsVersion4

// Specify a global access policy for all Workloads and Destinations associated with this VirtualMesh.
type VirtualMeshSpec_GlobalAccessPolicy int32

const (
	// Assume the default for the service mesh type. Istio defaults to `false`, App Mesh defaults to `true`.
	VirtualMeshSpec_MESH_DEFAULT VirtualMeshSpec_GlobalAccessPolicy = 0
	// Disallow traffic to all Destinations in the VirtualMesh unless explicitly allowed through [AccessPolicies]({{< versioned_link_path fromRoot="/reference/api/github.com.solo-io.gloo-mesh.api.networking.v1alpha2.access_policy/" >}}).
	VirtualMeshSpec_ENABLED VirtualMeshSpec_GlobalAccessPolicy = 1
	// Allow traffic to all Destinations in the VirtualMesh unless explicitly disallowed through [AccessPolicies]({{< versioned_link_path fromRoot="/reference/api/github.com.solo-io.gloo-mesh.api.networking.v1alpha2.access_policy/" >}}).
	VirtualMeshSpec_DISABLED VirtualMeshSpec_GlobalAccessPolicy = 2
)

// Enum value maps for VirtualMeshSpec_GlobalAccessPolicy.
var (
	VirtualMeshSpec_GlobalAccessPolicy_name = map[int32]string{
		0: "MESH_DEFAULT",
		1: "ENABLED",
		2: "DISABLED",
	}
	VirtualMeshSpec_GlobalAccessPolicy_value = map[string]int32{
		"MESH_DEFAULT": 0,
		"ENABLED":      1,
		"DISABLED":     2,
	}
)

func (x VirtualMeshSpec_GlobalAccessPolicy) Enum() *VirtualMeshSpec_GlobalAccessPolicy {
	p := new(VirtualMeshSpec_GlobalAccessPolicy)
	*p = x
	return p
}

func (x VirtualMeshSpec_GlobalAccessPolicy) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (VirtualMeshSpec_GlobalAccessPolicy) Descriptor() protoreflect.EnumDescriptor {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_enumTypes[0].Descriptor()
}

func (VirtualMeshSpec_GlobalAccessPolicy) Type() protoreflect.EnumType {
	return &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_enumTypes[0]
}

func (x VirtualMeshSpec_GlobalAccessPolicy) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use VirtualMeshSpec_GlobalAccessPolicy.Descriptor instead.
func (VirtualMeshSpec_GlobalAccessPolicy) EnumDescriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0, 0}
}

type VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism int32

const (
	VirtualMeshSpec_MTLSConfig_SharedTrust_SECRET      VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism = 0
	VirtualMeshSpec_MTLSConfig_SharedTrust_FILE_SYSTEM VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism = 1
)

// Enum value maps for VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism.
var (
	VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism_name = map[int32]string{
		0: "SECRET",
		1: "FILE_SYSTEM",
	}
	VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism_value = map[string]int32{
		"SECRET":      0,
		"FILE_SYSTEM": 1,
	}
)

func (x VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism) Enum() *VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism {
	p := new(VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism)
	*p = x
	return p
}

func (x VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism) Descriptor() protoreflect.EnumDescriptor {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_enumTypes[1].Descriptor()
}

func (VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism) Type() protoreflect.EnumType {
	return &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_enumTypes[1]
}

func (x VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism.Descriptor instead.
func (VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism) EnumDescriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0, 0, 0, 0}
}

// Represents a logical grouping of Meshes for shared configuration and cross-mesh interoperability.
type VirtualMeshSpec struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Specify the Meshes configured by this VirtualMesh.
	Meshes []*v1.ObjectRef `protobuf:"bytes,1,rep,name=meshes,proto3" json:"meshes,omitempty"`
	// Specify mTLS options.
	MtlsConfig *VirtualMeshSpec_MTLSConfig `protobuf:"bytes,2,opt,name=mtls_config,json=mtlsConfig,proto3" json:"mtls_config,omitempty"`
	// Specify how to federate Destinations across service mesh boundaries.
	Federation *VirtualMeshSpec_Federation `protobuf:"bytes,3,opt,name=federation,proto3" json:"federation,omitempty"`
	// Specify a global access policy for all Workloads and Destinations associated with this VirtualMesh.
	GlobalAccessPolicy VirtualMeshSpec_GlobalAccessPolicy `protobuf:"varint,4,opt,name=global_access_policy,json=globalAccessPolicy,proto3,enum=networking.mesh.gloo.solo.io.VirtualMeshSpec_GlobalAccessPolicy" json:"global_access_policy,omitempty"`
}

func (x *VirtualMeshSpec) Reset() {
	*x = VirtualMeshSpec{}
	if protoimpl.UnsafeEnabled {
		mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VirtualMeshSpec) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VirtualMeshSpec) ProtoMessage() {}

func (x *VirtualMeshSpec) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VirtualMeshSpec.ProtoReflect.Descriptor instead.
func (*VirtualMeshSpec) Descriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0}
}

func (x *VirtualMeshSpec) GetMeshes() []*v1.ObjectRef {
	if x != nil {
		return x.Meshes
	}
	return nil
}

func (x *VirtualMeshSpec) GetMtlsConfig() *VirtualMeshSpec_MTLSConfig {
	if x != nil {
		return x.MtlsConfig
	}
	return nil
}

func (x *VirtualMeshSpec) GetFederation() *VirtualMeshSpec_Federation {
	if x != nil {
		return x.Federation
	}
	return nil
}

func (x *VirtualMeshSpec) GetGlobalAccessPolicy() VirtualMeshSpec_GlobalAccessPolicy {
	if x != nil {
		return x.GlobalAccessPolicy
	}
	return VirtualMeshSpec_MESH_DEFAULT
}

type VirtualMeshStatus struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// The most recent generation observed in the the VirtualMesh metadata.
	// If the `observedGeneration` does not match `metadata.generation`, Gloo Mesh has not processed the most
	// recent version of this resource.
	ObservedGeneration int64 `protobuf:"varint,1,opt,name=observed_generation,json=observedGeneration,proto3" json:"observed_generation,omitempty"`
	// The state of the overall resource. It will only show accepted if it has been successfully
	// applied to all selected Meshes.
	State v11.ApprovalState `protobuf:"varint,2,opt,name=state,proto3,enum=common.mesh.gloo.solo.io.ApprovalState" json:"state,omitempty"`
	// Any errors found while processing this generation of the resource.
	Errors []string `protobuf:"bytes,3,rep,name=errors,proto3" json:"errors,omitempty"`
	// The status of the VirtualMesh for each Mesh to which it has been applied.
	// A VirtualMesh may be Accepted for some Meshes and rejected for others.
	Meshes map[string]*ApprovalStatus `protobuf:"bytes,4,rep,name=meshes,proto3" json:"meshes,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
	// The status of the VirtualMesh for each Destination to which it has been applied.
	// A VirtualMesh may be Accepted for some Destinations and rejected for others.
	Destinations map[string]*ApprovalStatus `protobuf:"bytes,5,rep,name=destinations,proto3" json:"destinations,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}

func (x *VirtualMeshStatus) Reset() {
	*x = VirtualMeshStatus{}
	if protoimpl.UnsafeEnabled {
		mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VirtualMeshStatus) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VirtualMeshStatus) ProtoMessage() {}

func (x *VirtualMeshStatus) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VirtualMeshStatus.ProtoReflect.Descriptor instead.
func (*VirtualMeshStatus) Descriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{1}
}

func (x *VirtualMeshStatus) GetObservedGeneration() int64 {
	if x != nil {
		return x.ObservedGeneration
	}
	return 0
}

func (x *VirtualMeshStatus) GetState() v11.ApprovalState {
	if x != nil {
		return x.State
	}
	return v11.ApprovalState_PENDING
}

func (x *VirtualMeshStatus) GetErrors() []string {
	if x != nil {
		return x.Errors
	}
	return nil
}

func (x *VirtualMeshStatus) GetMeshes() map[string]*ApprovalStatus {
	if x != nil {
		return x.Meshes
	}
	return nil
}

func (x *VirtualMeshStatus) GetDestinations() map[string]*ApprovalStatus {
	if x != nil {
		return x.Destinations
	}
	return nil
}

// Specify mTLS options.
// This includes options for configuring Mutual TLS within an individual mesh, as
// well as enabling mTLS across Meshes by establishing cross-mesh trust.
type VirtualMeshSpec_MTLSConfig struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Specify the model for establishing mTLS trust between Meshes.
	//
	// Types that are assignable to TrustModel:
	//	*VirtualMeshSpec_MTLSConfig_Shared
	//	*VirtualMeshSpec_MTLSConfig_Limited
	TrustModel isVirtualMeshSpec_MTLSConfig_TrustModel `protobuf_oneof:"trust_model"`
	// Specify whether to allow Gloo Mesh to restart Kubernetes Pods when certificates are rotated when establishing shared trust.
	// If this option is not explicitly enabled,
	// users must restart Pods manually for the new certificates to be picked up.
	// `meshctl` provides the command `meshctl mesh restart` to simplify this process, see [here]({{< versioned_link_path fromRoot="reference/cli/meshctl_mesh_restart/" >}}) for more info.
	AutoRestartPods bool `protobuf:"varint,3,opt,name=auto_restart_pods,json=autoRestartPods,proto3" json:"auto_restart_pods,omitempty"`
}

func (x *VirtualMeshSpec_MTLSConfig) Reset() {
	*x = VirtualMeshSpec_MTLSConfig{}
	if protoimpl.UnsafeEnabled {
		mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VirtualMeshSpec_MTLSConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VirtualMeshSpec_MTLSConfig) ProtoMessage() {}

func (x *VirtualMeshSpec_MTLSConfig) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VirtualMeshSpec_MTLSConfig.ProtoReflect.Descriptor instead.
func (*VirtualMeshSpec_MTLSConfig) Descriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0, 0}
}

func (m *VirtualMeshSpec_MTLSConfig) GetTrustModel() isVirtualMeshSpec_MTLSConfig_TrustModel {
	if m != nil {
		return m.TrustModel
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig) GetShared() *VirtualMeshSpec_MTLSConfig_SharedTrust {
	if x, ok := x.GetTrustModel().(*VirtualMeshSpec_MTLSConfig_Shared); ok {
		return x.Shared
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig) GetLimited() *VirtualMeshSpec_MTLSConfig_LimitedTrust {
	if x, ok := x.GetTrustModel().(*VirtualMeshSpec_MTLSConfig_Limited); ok {
		return x.Limited
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig) GetAutoRestartPods() bool {
	if x != nil {
		return x.AutoRestartPods
	}
	return false
}

type isVirtualMeshSpec_MTLSConfig_TrustModel interface {
	isVirtualMeshSpec_MTLSConfig_TrustModel()
}

type VirtualMeshSpec_MTLSConfig_Shared struct {
	// Shared trust (allow communication between any pair of Workloads and Destinations in the grouped Meshes).
	Shared *VirtualMeshSpec_MTLSConfig_SharedTrust `protobuf:"bytes,1,opt,name=shared,proto3,oneof"`
}

type VirtualMeshSpec_MTLSConfig_Limited struct {
	// Limited trust (selectively allow communication between Workloads and Destinations in the grouped Meshes). *Currently not available.*
	Limited *VirtualMeshSpec_MTLSConfig_LimitedTrust `protobuf:"bytes,2,opt,name=limited,proto3,oneof"`
}

func (*VirtualMeshSpec_MTLSConfig_Shared) isVirtualMeshSpec_MTLSConfig_TrustModel() {}

func (*VirtualMeshSpec_MTLSConfig_Limited) isVirtualMeshSpec_MTLSConfig_TrustModel() {}

// "Federation" refers to the ability to expose Destinations across service mesh boundaries, i.e. to traffic
// originating from Workloads external to the Destination's Mesh.
type VirtualMeshSpec_Federation struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Selectively federate Destinations to specific external meshes.
	// If omitted, all Destinations will be federated to all Meshes in the VirtualMesh.
	Selectors []*VirtualMeshSpec_Federation_FederationSelector `protobuf:"bytes,4,rep,name=selectors,proto3" json:"selectors,omitempty"`
	// DEPRECATED: Use `selectors` instead. Omission of the `selectors` field has permissive semantics.
	// The "mode" in which to federate Destinations within this VirtualMesh.
	//
	// Types that are assignable to Mode:
	//	*VirtualMeshSpec_Federation_Permissive
	Mode isVirtualMeshSpec_Federation_Mode `protobuf_oneof:"mode"`
	// If true, all multicluster traffic will be routed directly to the Kubernetes service endpoints of the Destinations,
	// rather than through an ingress gateway. This mode requires a flat network environment.
	// This feature is exclusive to Gloo Mesh Enterprise.
	FlatNetwork bool `protobuf:"varint,2,opt,name=flat_network,json=flatNetwork,proto3" json:"flat_network,omitempty"`
	// Configure the suffix for hostnames of Destinations federated within this VirtualMesh.
	// Currently this is only supported for Istio with [smart DNS proxying enabled](https://istio.io/latest/blog/2020/dns-proxy/),
	// otherwise setting this field results in an error.
	// If omitted, the hostname suffix defaults to "global".
	HostnameSuffix string `protobuf:"bytes,3,opt,name=hostname_suffix,json=hostnameSuffix,proto3" json:"hostname_suffix,omitempty"`
}

func (x *VirtualMeshSpec_Federation) Reset() {
	*x = VirtualMeshSpec_Federation{}
	if protoimpl.UnsafeEnabled {
		mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VirtualMeshSpec_Federation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VirtualMeshSpec_Federation) ProtoMessage() {}

func (x *VirtualMeshSpec_Federation) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VirtualMeshSpec_Federation.ProtoReflect.Descriptor instead.
func (*VirtualMeshSpec_Federation) Descriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0, 1}
}

func (x *VirtualMeshSpec_Federation) GetSelectors() []*VirtualMeshSpec_Federation_FederationSelector {
	if x != nil {
		return x.Selectors
	}
	return nil
}

func (m *VirtualMeshSpec_Federation) GetMode() isVirtualMeshSpec_Federation_Mode {
	if m != nil {
		return m.Mode
	}
	return nil
}

func (x *VirtualMeshSpec_Federation) GetPermissive() *empty.Empty {
	if x, ok := x.GetMode().(*VirtualMeshSpec_Federation_Permissive); ok {
		return x.Permissive
	}
	return nil
}

func (x *VirtualMeshSpec_Federation) GetFlatNetwork() bool {
	if x != nil {
		return x.FlatNetwork
	}
	return false
}

func (x *VirtualMeshSpec_Federation) GetHostnameSuffix() string {
	if x != nil {
		return x.HostnameSuffix
	}
	return ""
}

type isVirtualMeshSpec_Federation_Mode interface {
	isVirtualMeshSpec_Federation_Mode()
}

type VirtualMeshSpec_Federation_Permissive struct {
	// DEPRECATED: Use `selectors` instead. Omission of the `selectors` field has permissive semantics.
	// Expose all Destinations to all Workloads in this VirtualMesh.
	Permissive *empty.Empty `protobuf:"bytes,1,opt,name=permissive,proto3,oneof"`
}

func (*VirtualMeshSpec_Federation_Permissive) isVirtualMeshSpec_Federation_Mode() {}

// Shared trust is a trust model requiring a common root certificate shared between trusting Meshes, as well as shared identity
// between all Workloads and Destinations which wish to communicate within the VirtualMesh.
type VirtualMeshSpec_MTLSConfig_SharedTrust struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Types that are assignable to CertificateAuthority:
	//	*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_
	//	*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_
	CertificateAuthority isVirtualMeshSpec_MTLSConfig_SharedTrust_CertificateAuthority `protobuf_oneof:"certificate_authority"`
	// Configuration options for generated intermediate certs
	IntermediateCertOptions *v11.CommonCertOptions `protobuf:"bytes,3,opt,name=intermediate_cert_options,json=intermediateCertOptions,proto3" json:"intermediate_cert_options,omitempty"`
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust) Reset() {
	*x = VirtualMeshSpec_MTLSConfig_SharedTrust{}
	if protoimpl.UnsafeEnabled {
		mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust) ProtoMessage() {}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VirtualMeshSpec_MTLSConfig_SharedTrust.ProtoReflect.Descriptor instead.
func (*VirtualMeshSpec_MTLSConfig_SharedTrust) Descriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0, 0, 0}
}

func (m *VirtualMeshSpec_MTLSConfig_SharedTrust) GetCertificateAuthority() isVirtualMeshSpec_MTLSConfig_SharedTrust_CertificateAuthority {
	if m != nil {
		return m.CertificateAuthority
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust) GetRootCertificateAuthority() *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority {
	if x, ok := x.GetCertificateAuthority().(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_); ok {
		return x.RootCertificateAuthority
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust) GetIntermediateCertificateAuthority() *VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority {
	if x, ok := x.GetCertificateAuthority().(*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_); ok {
		return x.IntermediateCertificateAuthority
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust) GetIntermediateCertOptions() *v11.CommonCertOptions {
	if x != nil {
		return x.IntermediateCertOptions
	}
	return nil
}

type isVirtualMeshSpec_MTLSConfig_SharedTrust_CertificateAuthority interface {
	isVirtualMeshSpec_MTLSConfig_SharedTrust_CertificateAuthority()
}

type VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_ struct {
	// Configure a Root Certificate Authority which will be shared by all Meshes associated with this VirtualMesh.
	// If this is not provided, a self-signed certificate will be generated by Gloo Mesh.
	RootCertificateAuthority *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority `protobuf:"bytes,1,opt,name=root_certificate_authority,json=rootCertificateAuthority,proto3,oneof"`
}

type VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_ struct {
	IntermediateCertificateAuthority *VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority `protobuf:"bytes,2,opt,name=intermediate_certificate_authority,json=intermediateCertificateAuthority,proto3,oneof"`
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_) isVirtualMeshSpec_MTLSConfig_SharedTrust_CertificateAuthority() {
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_) isVirtualMeshSpec_MTLSConfig_SharedTrust_CertificateAuthority() {
}

// Limited trust is a trust model which does not require trusting Meshes to share the same root certificate
// or identity. Instead, trust is established between different Meshes
// by connecting their ingress/egress gateways with a common certificate/identity. In this model all requests
// between different have the following request path when communicating between clusters
// ```
//                cluster 1 MTLS               shared MTLS                  cluster 2 MTLS
// client/workload <-----------> egress gateway <----------> ingress gateway <--------------> server
// ```
// This approach has the downside of not maintaining identity from client to server, but allows for ad-hoc
// addition of additional Meshes into a VirtualMesh.
type VirtualMeshSpec_MTLSConfig_LimitedTrust struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *VirtualMeshSpec_MTLSConfig_LimitedTrust) Reset() {
	*x = VirtualMeshSpec_MTLSConfig_LimitedTrust{}
	if protoimpl.UnsafeEnabled {
		mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VirtualMeshSpec_MTLSConfig_LimitedTrust) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VirtualMeshSpec_MTLSConfig_LimitedTrust) ProtoMessage() {}

func (x *VirtualMeshSpec_MTLSConfig_LimitedTrust) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VirtualMeshSpec_MTLSConfig_LimitedTrust.ProtoReflect.Descriptor instead.
func (*VirtualMeshSpec_MTLSConfig_LimitedTrust) Descriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0, 0, 1}
}

// Specify parameters for configuring the root certificate authority for a VirtualMesh.
type VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Specify the source of the Root CA data which Gloo Mesh will use for the VirtualMesh.
	//
	// Types that are assignable to CaSource:
	//	*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Secret
	//	*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Vault
	CaSource         isVirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_CaSource `protobuf_oneof:"ca_source"`
	StorageMechanism VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism                            `protobuf:"varint,3,opt,name=storage_mechanism,json=storageMechanism,proto3,enum=networking.mesh.gloo.solo.io.VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism" json:"storage_mechanism,omitempty"`
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority) Reset() {
	*x = VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority{}
	if protoimpl.UnsafeEnabled {
		mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority) ProtoMessage() {}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority.ProtoReflect.Descriptor instead.
func (*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority) Descriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0, 0, 0, 0}
}

func (m *VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority) GetCaSource() isVirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_CaSource {
	if m != nil {
		return m.CaSource
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority) GetSecret() *v1.ObjectRef {
	if x, ok := x.GetCaSource().(*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Secret); ok {
		return x.Secret
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority) GetVault() *v11.VaultCA {
	if x, ok := x.GetCaSource().(*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Vault); ok {
		return x.Vault
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority) GetStorageMechanism() VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism {
	if x != nil {
		return x.StorageMechanism
	}
	return VirtualMeshSpec_MTLSConfig_SharedTrust_SECRET
}

type isVirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_CaSource interface {
	isVirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_CaSource()
}

type VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Secret struct {
	// Reference to a Kubernetes Secret containing the root certificate authority.
	// Provided secrets must conform to a specified format, [documented here]({{< versioned_link_path fromRoot="/guides/federate_identity/" >}}).
	Secret *v1.ObjectRef `protobuf:"bytes,1,opt,name=secret,proto3,oneof"`
}

type VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Vault struct {
	Vault *v11.VaultCA `protobuf:"bytes,2,opt,name=vault,proto3,oneof"`
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Secret) isVirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_CaSource() {
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Vault) isVirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_CaSource() {
}

// Specify parameters for configuring the root certificate authority for a VirtualMesh.
type VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Specify the source of the Root CA data which Gloo Mesh will use for the VirtualMesh.
	//
	// Types that are assignable to CaSource:
	//	*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Generated
	//	*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Secret
	//	*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Vault
	CaSource         isVirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_CaSource `protobuf_oneof:"ca_source"`
	StorageMechanism VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism                    `protobuf:"varint,4,opt,name=storage_mechanism,json=storageMechanism,proto3,enum=networking.mesh.gloo.solo.io.VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism" json:"storage_mechanism,omitempty"`
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) Reset() {
	*x = VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority{}
	if protoimpl.UnsafeEnabled {
		mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) ProtoMessage() {}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority.ProtoReflect.Descriptor instead.
func (*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) Descriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0, 0, 0, 1}
}

func (m *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) GetCaSource() isVirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_CaSource {
	if m != nil {
		return m.CaSource
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) GetGenerated() *v11.CommonCertOptions {
	if x, ok := x.GetCaSource().(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Generated); ok {
		return x.Generated
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) GetSecret() *v1.ObjectRef {
	if x, ok := x.GetCaSource().(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Secret); ok {
		return x.Secret
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) GetVault() *v11.VaultCA {
	if x, ok := x.GetCaSource().(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Vault); ok {
		return x.Vault
	}
	return nil
}

func (x *VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority) GetStorageMechanism() VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism {
	if x != nil {
		return x.StorageMechanism
	}
	return VirtualMeshSpec_MTLSConfig_SharedTrust_SECRET
}

type isVirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_CaSource interface {
	isVirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_CaSource()
}

type VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Generated struct {
	// Generate a self-signed root certificate with the given options.
	Generated *v11.CommonCertOptions `protobuf:"bytes,1,opt,name=generated,proto3,oneof"`
}

type VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Secret struct {
	// Reference to a Kubernetes Secret containing the root certificate authority.
	// Provided secrets must conform to a specified format, [documented here]({{< versioned_link_path fromRoot="/guides/federate_identity/" >}}).
	Secret *v1.ObjectRef `protobuf:"bytes,2,opt,name=secret,proto3,oneof"`
}

type VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Vault struct {
	Vault *v11.VaultCA `protobuf:"bytes,3,opt,name=vault,proto3,oneof"`
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Generated) isVirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_CaSource() {
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Secret) isVirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_CaSource() {
}

func (*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Vault) isVirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_CaSource() {
}

// Selects a set of Destinations to federate to the referenced Meshes.
type VirtualMeshSpec_Federation_FederationSelector struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// The set of Destinations that will be federated to external Meshes.
	// If omitted, all Destinations will be selected.
	DestinationSelectors []*v11.DestinationSelector `protobuf:"bytes,1,rep,name=destination_selectors,json=destinationSelectors,proto3" json:"destination_selectors,omitempty"`
	// The Meshes to which the selected Destinations will be federated. All referenced Meshes must exist in this VirtualMesh.
	// If omitted, the selected Destinations will be federated to all Meshes in the VirtualMesh.
	Meshes []*v1.ObjectRef `protobuf:"bytes,2,rep,name=meshes,proto3" json:"meshes,omitempty"`
}

func (x *VirtualMeshSpec_Federation_FederationSelector) Reset() {
	*x = VirtualMeshSpec_Federation_FederationSelector{}
	if protoimpl.UnsafeEnabled {
		mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VirtualMeshSpec_Federation_FederationSelector) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VirtualMeshSpec_Federation_FederationSelector) ProtoMessage() {}

func (x *VirtualMeshSpec_Federation_FederationSelector) ProtoReflect() protoreflect.Message {
	mi := &file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VirtualMeshSpec_Federation_FederationSelector.ProtoReflect.Descriptor instead.
func (*VirtualMeshSpec_Federation_FederationSelector) Descriptor() ([]byte, []int) {
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP(), []int{0, 1, 0}
}

func (x *VirtualMeshSpec_Federation_FederationSelector) GetDestinationSelectors() []*v11.DestinationSelector {
	if x != nil {
		return x.DestinationSelectors
	}
	return nil
}

func (x *VirtualMeshSpec_Federation_FederationSelector) GetMeshes() []*v1.ObjectRef {
	if x != nil {
		return x.Meshes
	}
	return nil
}

var File_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto protoreflect.FileDescriptor

var file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDesc = []byte{
	,
}

var (
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescOnce sync.Once
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescData = file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDesc
)

func file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescGZIP() []byte {
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescOnce.Do(func() {
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescData = protoimpl.X.CompressGZIP(file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescData)
	})
	return file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDescData
}

var file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes = make([]protoimpl.MessageInfo, 11)
var file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_goTypes = []interface{}{
	(VirtualMeshSpec_GlobalAccessPolicy)(0),                                         // 0: networking.mesh.gloo.solo.io.VirtualMeshSpec.GlobalAccessPolicy
	(VirtualMeshSpec_MTLSConfig_SharedTrust_StorageMechanism)(0),                    // 1: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.StorageMechanism
	(*VirtualMeshSpec)(nil),                                                         // 2: networking.mesh.gloo.solo.io.VirtualMeshSpec
	(*VirtualMeshStatus)(nil),                                                       // 3: networking.mesh.gloo.solo.io.VirtualMeshStatus
	(*VirtualMeshSpec_MTLSConfig)(nil),                                              // 4: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig
	(*VirtualMeshSpec_Federation)(nil),                                              // 5: networking.mesh.gloo.solo.io.VirtualMeshSpec.Federation
	(*VirtualMeshSpec_MTLSConfig_SharedTrust)(nil),                                  // 6: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust
	(*VirtualMeshSpec_MTLSConfig_LimitedTrust)(nil),                                 // 7: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.LimitedTrust
	(*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority)(nil), // 8: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.IntermediateCertificateAuthority
	(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority)(nil),         // 9: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.RootCertificateAuthority
	(*VirtualMeshSpec_Federation_FederationSelector)(nil),                           // 10: networking.mesh.gloo.solo.io.VirtualMeshSpec.Federation.FederationSelector
	nil,                             // 11: networking.mesh.gloo.solo.io.VirtualMeshStatus.MeshesEntry
	nil,                             // 12: networking.mesh.gloo.solo.io.VirtualMeshStatus.DestinationsEntry
	(*v1.ObjectRef)(nil),            // 13: core.skv2.solo.io.ObjectRef
	(v11.ApprovalState)(0),          // 14: common.mesh.gloo.solo.io.ApprovalState
	(*empty.Empty)(nil),             // 15: google.protobuf.Empty
	(*v11.CommonCertOptions)(nil),   // 16: common.mesh.gloo.solo.io.CommonCertOptions
	(*v11.VaultCA)(nil),             // 17: common.mesh.gloo.solo.io.VaultCA
	(*v11.DestinationSelector)(nil), // 18: common.mesh.gloo.solo.io.DestinationSelector
	(*ApprovalStatus)(nil),          // 19: networking.mesh.gloo.solo.io.ApprovalStatus
}
var file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_depIdxs = []int32{
	13, // 0: networking.mesh.gloo.solo.io.VirtualMeshSpec.meshes:type_name -> core.skv2.solo.io.ObjectRef
	4,  // 1: networking.mesh.gloo.solo.io.VirtualMeshSpec.mtls_config:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig
	5,  // 2: networking.mesh.gloo.solo.io.VirtualMeshSpec.federation:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.Federation
	0,  // 3: networking.mesh.gloo.solo.io.VirtualMeshSpec.global_access_policy:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.GlobalAccessPolicy
	14, // 4: networking.mesh.gloo.solo.io.VirtualMeshStatus.state:type_name -> common.mesh.gloo.solo.io.ApprovalState
	11, // 5: networking.mesh.gloo.solo.io.VirtualMeshStatus.meshes:type_name -> networking.mesh.gloo.solo.io.VirtualMeshStatus.MeshesEntry
	12, // 6: networking.mesh.gloo.solo.io.VirtualMeshStatus.destinations:type_name -> networking.mesh.gloo.solo.io.VirtualMeshStatus.DestinationsEntry
	6,  // 7: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.shared:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust
	7,  // 8: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.limited:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.LimitedTrust
	10, // 9: networking.mesh.gloo.solo.io.VirtualMeshSpec.Federation.selectors:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.Federation.FederationSelector
	15, // 10: networking.mesh.gloo.solo.io.VirtualMeshSpec.Federation.permissive:type_name -> google.protobuf.Empty
	9,  // 11: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.root_certificate_authority:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.RootCertificateAuthority
	8,  // 12: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.intermediate_certificate_authority:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.IntermediateCertificateAuthority
	16, // 13: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.intermediate_cert_options:type_name -> common.mesh.gloo.solo.io.CommonCertOptions
	13, // 14: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.IntermediateCertificateAuthority.secret:type_name -> core.skv2.solo.io.ObjectRef
	17, // 15: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.IntermediateCertificateAuthority.vault:type_name -> common.mesh.gloo.solo.io.VaultCA
	1,  // 16: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.IntermediateCertificateAuthority.storage_mechanism:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.StorageMechanism
	16, // 17: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.RootCertificateAuthority.generated:type_name -> common.mesh.gloo.solo.io.CommonCertOptions
	13, // 18: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.RootCertificateAuthority.secret:type_name -> core.skv2.solo.io.ObjectRef
	17, // 19: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.RootCertificateAuthority.vault:type_name -> common.mesh.gloo.solo.io.VaultCA
	1,  // 20: networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.RootCertificateAuthority.storage_mechanism:type_name -> networking.mesh.gloo.solo.io.VirtualMeshSpec.MTLSConfig.SharedTrust.StorageMechanism
	18, // 21: networking.mesh.gloo.solo.io.VirtualMeshSpec.Federation.FederationSelector.destination_selectors:type_name -> common.mesh.gloo.solo.io.DestinationSelector
	13, // 22: networking.mesh.gloo.solo.io.VirtualMeshSpec.Federation.FederationSelector.meshes:type_name -> core.skv2.solo.io.ObjectRef
	19, // 23: networking.mesh.gloo.solo.io.VirtualMeshStatus.MeshesEntry.value:type_name -> networking.mesh.gloo.solo.io.ApprovalStatus
	19, // 24: networking.mesh.gloo.solo.io.VirtualMeshStatus.DestinationsEntry.value:type_name -> networking.mesh.gloo.solo.io.ApprovalStatus
	25, // [25:25] is the sub-list for method output_type
	25, // [25:25] is the sub-list for method input_type
	25, // [25:25] is the sub-list for extension type_name
	25, // [25:25] is the sub-list for extension extendee
	0,  // [0:25] is the sub-list for field type_name
}

func init() { file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_init() }
func file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_init() {
	if File_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto != nil {
		return
	}
	file_github_com_solo_io_gloo_mesh_api_networking_v1_status_proto_init()
	if !protoimpl.UnsafeEnabled {
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VirtualMeshSpec); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VirtualMeshStatus); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VirtualMeshSpec_MTLSConfig); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VirtualMeshSpec_Federation); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VirtualMeshSpec_MTLSConfig_SharedTrust); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VirtualMeshSpec_MTLSConfig_LimitedTrust); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[8].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VirtualMeshSpec_Federation_FederationSelector); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[2].OneofWrappers = []interface{}{
		(*VirtualMeshSpec_MTLSConfig_Shared)(nil),
		(*VirtualMeshSpec_MTLSConfig_Limited)(nil),
	}
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[3].OneofWrappers = []interface{}{
		(*VirtualMeshSpec_Federation_Permissive)(nil),
	}
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[4].OneofWrappers = []interface{}{
		(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_)(nil),
		(*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_)(nil),
	}
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[6].OneofWrappers = []interface{}{
		(*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Secret)(nil),
		(*VirtualMeshSpec_MTLSConfig_SharedTrust_IntermediateCertificateAuthority_Vault)(nil),
	}
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes[7].OneofWrappers = []interface{}{
		(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Generated)(nil),
		(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Secret)(nil),
		(*VirtualMeshSpec_MTLSConfig_SharedTrust_RootCertificateAuthority_Vault)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDesc,
			NumEnums:      2,
			NumMessages:   11,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_goTypes,
		DependencyIndexes: file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_depIdxs,
		EnumInfos:         file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_enumTypes,
		MessageInfos:      file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_msgTypes,
	}.Build()
	File_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto = out.File
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_rawDesc = nil
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_goTypes = nil
	file_github_com_solo_io_gloo_mesh_api_networking_v1_virtual_mesh_proto_depIdxs = nil
}
