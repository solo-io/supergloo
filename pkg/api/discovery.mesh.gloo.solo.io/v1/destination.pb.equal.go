...
func (m *DestinationStatus) Equal(that interface{}) bool {
	...
	if len(m.GetAppliedSubsets()) != len(target.GetAppliedSubsets()) {
		return false
	}
	for idx, v := range m.GetAppliedSubsets() {

		if h, ok := interface{}(v).(equality.Equalizer); ok {
			if !h.Equal(target.GetAppliedSubsets()[idx]) {
				return false
			}
		} else {
			if !proto.Equal(v, target.GetAppliedSubsets()[idx]) {
				return false
			}
		}
...
// Equal function
func (m *DestinationStatus_AppliedSubsets) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*DestinationStatus_AppliedSubsets)
	if !ok {
		that2, ok := that.(DestinationStatus_AppliedSubsets)
		if ok {
			target = &that2
		} else {
			return false
