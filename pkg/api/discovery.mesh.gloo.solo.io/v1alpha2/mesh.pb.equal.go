...
func (m *MeshSpec_AwsAppMesh) Equal(that interface{}) bool {
	...
	if len(m.GetClusterMeshResources()) != len(target.GetClusterMeshResources()) {
		return false
	}
	for k, v := range m.GetClusterMeshResources() {

		if h, ok := interface{}(v).(equality.Equalizer); ok {
			if !h.Equal(target.GetClusterMeshResources()[k]) {
				return false
			}
		} else {
			if !proto.Equal(v, target.GetClusterMeshResources()[k]) {
				return false
			}
		}
...
// Equal function
func (m *MeshSpec_AwsAppMesh_MeshRef) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*MeshSpec_AwsAppMesh_MeshRef)
	if !ok {
