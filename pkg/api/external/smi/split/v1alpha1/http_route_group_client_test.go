...
var _ = Describe("HTTPRouteGroupClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: HTTPRouteGroupCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewHTTPRouteGroupClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs HTTPRouteGroups "+test.Description(), func() {
				HTTPRouteGroupClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func HTTPRouteGroupClientTest(namespace string, client HTTPRouteGroupClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewHTTPRouteGroup(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&HTTPRouteGroup{}))
	...
	Expect(r1.Matches).To(Equal(input.Matches))
	...
	Eventually(func() HTTPRouteGroupList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
