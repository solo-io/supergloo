...
var _ = Describe("TrafficSplitClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: TrafficSplitCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewTrafficSplitClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs TrafficSplits "+test.Description(), func() {
				TrafficSplitClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func TrafficSplitClientTest(namespace string, client TrafficSplitClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewTrafficSplit(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&TrafficSplit{}))
	...
	Eventually(func() TrafficSplitList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
