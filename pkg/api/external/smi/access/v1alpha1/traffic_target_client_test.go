...
var _ = Describe("TrafficTargetClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: TrafficTargetCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewTrafficTargetClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs TrafficTargets "+test.Description(), func() {
				TrafficTargetClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func TrafficTargetClientTest(namespace string, client TrafficTargetClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewTrafficTarget(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&TrafficTarget{}))
	...
	Expect(r1.Destination).To(Equal(input.Destination))
	Expect(r1.Sources).To(Equal(input.Sources))
	Expect(r1.Specs).To(Equal(input.Specs))
	...
	Eventually(func() TrafficTargetList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
