	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewMeshPolicyClient(factory)
			...
			It("CRUDs MeshPolicys "+test.Description(), func() {
				MeshPolicyClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func MeshPolicyClientTest(namespace string, client MeshPolicyClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewMeshPolicy(namespace, name)
