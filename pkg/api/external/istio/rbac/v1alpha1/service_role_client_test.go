...
var _ = Describe("ServiceRoleClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs ServiceRoles "+test.Description(), func() {
				ServiceRoleClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func ServiceRoleClientTest(namespace string, client ServiceRoleClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
	...
	Eventually(func() ServiceRoleList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
