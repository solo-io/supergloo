...
var _ = Describe("ServiceRoleBindingClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs ServiceRoleBindings "+test.Description(), func() {
				ServiceRoleBindingClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func ServiceRoleBindingClientTest(namespace string, client ServiceRoleBindingClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
	...
	Eventually(func() ServiceRoleBindingList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
