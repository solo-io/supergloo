...
var _ = Describe("DestinationRuleClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: DestinationRuleCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewDestinationRuleClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs DestinationRules "+test.Description(), func() {
				DestinationRuleClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func DestinationRuleClientTest(namespace string, client DestinationRuleClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewDestinationRule(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&DestinationRule{}))
	...
	Eventually(func() DestinationRuleList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
