...
var _ = Describe("VirtualServiceClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs VirtualServices "+test.Description(), func() {
				VirtualServiceClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func VirtualServiceClientTest(namespace string, client VirtualServiceClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
	...
	Eventually(func() VirtualServiceList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
