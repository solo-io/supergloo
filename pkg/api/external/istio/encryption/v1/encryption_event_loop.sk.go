...
type EncryptionSyncer interface {
	Sync(context.Context, *EncryptionSnapshot) error
}

type EncryptionSyncers []EncryptionSyncer

func (s EncryptionSyncers) Sync(ctx context.Context, snapshot *EncryptionSnapshot) error {
...
type EncryptionEventLoop interface {
	Run(namespaces []string, opts clients.WatchOpts) (<-chan error, error)
}

type encryptionEventLoop struct {
	emitter EncryptionEmitter
	syncer  EncryptionSyncer
}

func NewEncryptionEventLoop(emitter EncryptionEmitter, syncer EncryptionSyncer) EncryptionEventLoop {
	return &encryptionEventLoop{
		emitter: emitter,
		syncer:  syncer,
	}
}

func (el *encryptionEventLoop) Run(namespaces []string, opts clients.WatchOpts) (<-chan error, error) {
	...
	go func() {
		...
		for {
			...
			case snapshot, ok := <-watch:
				...
				ctx, span := trace.StartSpan(opts.Ctx, "encryption.istio.io.EventLoopSync")
