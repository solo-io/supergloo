...
type VirtualMeshLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.VirtualMesh, err error)
...
// List lists all VirtualMeshes in the indexer.
func (s *virtualMeshLister) List(selector labels.Selector) (ret []*v1alpha1.VirtualMesh, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.VirtualMesh))
	})
	return ret, err
}
...
type VirtualMeshNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.VirtualMesh, error)
...
// List lists all VirtualMeshes in the indexer for a given namespace.
func (s virtualMeshNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.VirtualMesh, err error) {
...
// Get retrieves the VirtualMesh from the indexer for a given namespace and name.
func (s virtualMeshNamespaceLister) Get(name string) (*v1alpha1.VirtualMesh, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("virtualmesh"), name)
	}
	return obj.(*v1alpha1.VirtualMesh), nil
}
