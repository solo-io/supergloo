...
var _ = Describe("NetworkingTranslator", func() {
	...
	It("subsequent translations should update outputs with new objects and garbage collect orphaned objects", func() {
		...
		vs1 := virtualService(
			"vs1",
			[]string{"host1"},
			map[client.Object]schema.GroupVersionKind{destination1: discoveryv1.DestinationGVK},
		)
		...
		dr1 := destinationRule(
			"dr1",
			map[client.Object]schema.GroupVersionKind{
				destination1:   discoveryv1.DestinationGVK,
				trafficPolicy1: networkingv1.TrafficPolicyGVK,
			},
		)
		...
		mockIstioTranslator.
			...
			Do(func(
				...
				istioOutputs.AddDestinationRules(dr1)
			})
		mockAppMeshTranslator.
			EXPECT().
		...
		Expect(outputs.Istio.GetDestinationRules().List()).To(ConsistOf([]*networkingv1alpha3.DestinationRule{dr1}))
		...
		Expect(outputs.Istio.GetDestinationRules().List()).To(BeEmpty())
	})
})
...
func virtualService(
	name string,
	hosts []string,
	parents map[client.Object]schema.GroupVersionKind,
	...
	for parent, parentGvk := range parents {
		metautils.AppendParent(context.TODO(), vs, parent, parentGvk)
	}
...
func destinationRule(
	...
	for parent, parentGvk := range parents {
		metautils.AppendParent(context.TODO(), dr, parent, parentGvk)
	}
