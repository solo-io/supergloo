...
var (
	VirtualMeshTrustBundleNotFoundMsg = func(err error, ref *smh_core_types.ResourceRef) error {
...
func (c *certSinger) Sign(
	...
	obj.Status.Response = &smh_security_types.VirtualMeshCertificateSigningRequestStatus_Response{
		CaCertificate:   cert,
...
func (c *certSinger) shouldProcess(csr *smh_security.VirtualMeshCertificateSigningRequest) bool {
	// TODO: make this configurable so third party workflows can be enabled
	switch {
	// Third party approval is not in the correct state
	case csr.Status.GetThirdPartyApproval().GetApprovalStatus() != smh_security_types.VirtualMeshCertificateSigningRequestStatus_ThirdPartyApprovalWorkflow_APPROVED &&
		csr.Status.GetThirdPartyApproval().GetApprovalStatus() != smh_security_types.VirtualMeshCertificateSigningRequestStatus_ThirdPartyApprovalWorkflow_PENDING:
		return false
...
func (c *certSinger) ProcessDelete(
	ctx context.Context,
	csr *smh_security.VirtualMeshCertificateSigningRequest,
) (computedStatus *smh_security_types.VirtualMeshCertificateSigningRequestStatus) {
	return
}
