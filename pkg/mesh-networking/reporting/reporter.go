...
type Reporter interface {
	...
	// report an error on a list of traffic policies that has been applied to a MeshService
	ReportTrafficPolicies(meshService *v1alpha1.MeshService, trafficPolicies []ezkube.ResourceId, err error)
...
func (p *panickingReporter) ReportTrafficPolicies(meshService *v1alpha1.MeshService, trafficPolicies []ezkube.ResourceId, err error) {
	contextutils.LoggerFrom(p.ctx).
		DPanicw(
			"internal error: error reported on TrafficPolicy which should have been caught by validation!",
			"policies", resourceidutils.ResourceIdsToString(trafficPolicies),
			"mesh-service", sets.Key(meshService),
			"error", err)
}
