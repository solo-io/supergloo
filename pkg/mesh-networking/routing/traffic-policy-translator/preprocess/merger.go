...
import (
	...
	networking_errors "github.com/solo-io/service-mesh-hub/pkg/mesh-networking/routing/traffic-policy-translator/errors"
)
...
func (t *trafficPolicyMerger) getTrafficPoliciesByMeshService(
	...
	trafficPoliciesByMeshService := map[selection.MeshServiceId][]*smh_networking.TrafficPolicy{}
	...
	for _, meshService := range meshServices {
		...
		trafficPoliciesByMeshService[*meshServiceKey] = []*smh_networking.TrafficPolicy{}
	}
...
func mergeTrafficPoliciesByMeshService(
	...
	for meshServiceKey, trafficPolicies := range trafficPoliciesByMeshService {
		...
		for _, trafficPolicy := range trafficPolicies {
			...
			if trafficPolicy.Status.GetTranslationStatus().GetState() == smh_core_types.Status_ACCEPTED {
				...
				*/
				if mergeableHttpTp.HttpMatcher != nil {
					newMergedTrafficPolicy.Spec.HttpRequestMatchers =
						[]*smh_networking_types.TrafficPolicySpec_HttpMatcher{mergeableHttpTp.HttpMatcher}
				}
			}
		}
...
func attemptTrafficPolicyMerge(
	...
	newTPSpec, err := mergeTrafficPolicySpec(&smh_networking_types.TrafficPolicySpec{}, &trafficPolicy.Spec)
	if err != nil {
		return nil, err
	}
