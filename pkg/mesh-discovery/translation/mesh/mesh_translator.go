...
type Translator interface {
	TranslateMeshes(inRemote input.RemoteSnapshot, inLocal input.LocalSnapshot) v1alpha2sets.MeshSet
}
...
func (t *translator) TranslateMeshes(inRemote input.RemoteSnapshot, inLocal input.LocalSnapshot) v1alpha2sets.MeshSet {
	meshSet := v1alpha2sets.NewMeshSet()
	meshes, err := t.meshDetector.DetectMeshes(inRemote, inLocal)
	if err != nil {
