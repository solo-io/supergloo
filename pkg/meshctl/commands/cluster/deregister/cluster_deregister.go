...
func (opts *deregistrationOptions) addToFlags(set *pflag.FlagSet) {
	kubeConfigPath := set.String("kubeconfig", "", "path to the kubeconfig from which the registered cluster will be accessed")
	mgmtContext := set.String("mgmt-context", "", "name of the kubeconfig context to use for the management cluster")
	remoteContext := set.String("remote-context", "", "name of the kubeconfig context to use for the remote cluster")
	...
	opts.KubeCfg = register.NewDiskKubeCfg(*kubeConfigPath, *mgmtContext)
	opts.RemoteKubeCfg = register.NewDiskKubeCfg(*kubeConfigPath, *remoteContext)
}
