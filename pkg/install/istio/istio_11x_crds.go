...
var istio113ExtraManifests = func() helmchart.Manifests {
	manifests := manifest.SplitManifests(map[string]string{
		"istio-1.1.3-crds.yaml": istio113CrdManifestContent,
	})
	var convertedManifests helmchart.Manifests
	for _, man := range manifests {
		convertedManifests = append(convertedManifests, man)
	}
	return convertedManifests
}()

// these crd manifests copied from Istio's 1.1.3 chart: istio-init/files/crd-*.yaml
const istio113CrdManifestContent = `
