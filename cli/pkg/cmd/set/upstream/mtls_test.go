...
var _ = Describe("edit upstream", func() {
	...
	Context("validation", func() {
		It("returns err if name or namespace of mesh aren't present", func() {
			err := utils.Supergloo("set upstream mtls")
		...
		It("returns err if name or namespace of upstream aren't present", func() {
			err := utils.Supergloo("set upstream mtls --namespace one --target-mesh one.two")
			Expect(err).To(HaveOccurred())
		...
		It("must be a valid mesh target", func() {
			...
			err = utils.Supergloo("set upstream mtls --name one --target-mesh one.two")
			Expect(err).To(HaveOccurred())
	...
	Context("successful edits", func() {
		...
		It("can edit an upstream", func() {
			err := utils.Supergloo(fmt.Sprintf("set upstream mtls --name one --target-mesh %s.%s", ns, name))
			Expect(err).NotTo(HaveOccurred())
