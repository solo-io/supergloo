...
var _ = Describe("SurveyIstioInstall", func() {
	It("should create the expected istio install ", func() {
		...
		testutil.ExpectInteractive(func(c *testutil.Console) {
			...
			c.SendLine("y")
			c.ExpectString("enable ingress gateway? ")
			c.SendLine("y")
			c.ExpectString("enable egress gateway? ")
			c.SendLine("y")
			...
			Expect(in.IstioInstall.Version).To(Equal(istio.IstioVersion105))
			Expect(in.IstioInstall.EnableMtls).To(Equal(true))
			Expect(in.IstioInstall.EnableAutoInject).To(Equal(true))
			Expect(in.IstioInstall.EnableIngress).To(Equal(true))
			Expect(in.IstioInstall.EnableEgress).To(Equal(true))
...
var _ = Describe("SurveyLinkerdInstall", func() {
	It("should create the expected linkerd install ", func() {
		...
		testutil.ExpectInteractive(func(c *testutil.Console) {
			...
			Expect(in.LinkerdInstall.Version).To(Equal(linkerd.Version_stable230))
...
var _ = Describe("SurveyGlooInstall", func() {
	...
	It("should create the expected gloo install ", func() {
		...
		testutil.ExpectInteractive(func(c *testutil.Console) {
			...
			Expect(in.GlooIngressInstall.Version).To(Equal("latest"))
